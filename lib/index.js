...
function pushApp (db, appDir, rev, callback) {
  ...
  CouchApp(appDir, function (err, doc) {
    ...
    doc.views = {
      ...
      by_artist: {
        map: (function (doc) {
          if ('title' in doc && 'album' in doc && 'artist' in doc) {
            emit([doc.artist, doc.album, doc.title], 1);
          }
        }
      }
    };
    // Cloudant.com compatible Search Index including Facets for title, album
    // and artist.
    // How many artists have covered "All Along the Watchtower" (by Bob Dylan)?
    // ./_search/songs?q=title:"All Along the Watchtower"&counts=["artist"]
    doc.indexes = {
      songs: (function (doc) {
        var all = [];
        function facet_these(keys) {
          keys.forEach(function(key) {
            if (key in doc) {
              all.push(doc[key]);
              index(key, doc[key], {store: true, facet: true});
            }
          });
        }
        facet_these(['title', 'album', 'artist']);
        index('default', all.join(' '));
      }).toString()
    },
    doc.rewrites = [
      { from: "api" , to: "../../"},
    ...
    pushApp(db, appDir, appRev, function (err, appResponse) {
      ...
      readDir(directory, function (err, tracks) {
        ...
        async.reduce(tracks, [], function (acc, track, done) {
          ...
          async.mapSeries(utils.group(docs, 8), function (set, done) {
