    ...
    pushApp(db, appDir, appRev, function (err, appResponse) {
      ...
      readDir(directory, function (err, tracks) {
        ...
        async.reduce(tracks, [], function (acc, track, done) {
          ...
          if (err) return callback.call(null, err);
          utils.log('uploading library. This make take a few minutes...')
          // To avoid 413 Request Entity Too Large, split docs into
          // subsets aiming for < 64MB.
          async.map(docs.group(8), function (set, done) {
