 ...
 *
 * @param {string} track
 * @param {function} callback (err, metadata, b64data)
 */
    ...
    pushApp(db, appDir, function (err, response) {
      readDir(directory, function (err, tracks) {
        ...
        async.each(tracks, function (track, done) {
          getTrackData(track, function (err, metadata, b64data) {
            ...
            metadata['_id'] = 'meta_' + metadata['_id'];
            db.bulkDocs({ docs: [metadata, fileDoc] }, function (err, res) {
              if (err) throw err;
            });
          });
        }, callback);
