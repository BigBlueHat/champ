...
/**
 * Uploads the client side CouchApp to the database from ../client/
 *
 * @param {string} uri
 * @param {function} callback
 */

function uploadCouchApp (uri, callback) {
  var ddoc = {
    _id: '_design/hoax',
    views: {},
    lists: {},
  ...
  couchapp.loadAttachments(ddoc, path.resolve(__dirname, '../client'));
  couchapp.createApp(ddoc, uri, function (app) {
    app.push(callback);
  });
}
